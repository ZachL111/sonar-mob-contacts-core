...
`sonar-mob-contacts-core` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for contacts workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 177. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 3.
